...
class post_img_handle_ap {
	...
	public static function down_remote_img($post_title, $url, $referer, $minWidth, $useProxy = 0, $proxy = null, $downImgTimeOut = 120, $relativeURL = 0, $downFileOrganize = 0, $cookie = null, $cookiejar = null) {
		$url = html_entity_decode(trim($url));
		$url = getRawUrl($url);
		if (function_exists('curl_init')) {
			$result = self :: down_remote_img_by_curl($post_title, $url, $referer, $minWidth, $useProxy, $proxy, $downImgTimeOut, $relativeURL, $downFileOrganize, $cookie, $cookiejar);
			if ($result['try_use_wp']) {
				$result = self :: down_remote_img_by_wp($post_title, $url, $minWidth, $downImgTimeOut, $relativeURL, $downFileOrganize);
			} 
		} else {
			$result = self :: down_remote_img_by_wp($post_title, $url, $minWidth, $downImgTimeOut, $relativeURL, $downFileOrganize);
		} 
	...
	public static function down_remote_img_by_curl($post_title, $url, $referer, $minWidth, $useProxy = 0, $proxy = null, $downImgTimeOut = 120, $relativeURL = 0, $downFileOrganize = 0, $cookie = null, $cookiejar = null) {
		...
		if (in_array ($file_ext['ext'], $allowed_filetype)) {
			if ($file_ext['check_size']) {
				if ($minWidth > 0) {
					$img_res = imagecreatefromstring ($rs);
					$width = imagesx ($img_res);
			...
			$result = self :: handle_upload_img ($or_filename, $rs, $mime, $file_ext['ext'], $relativeURL, $downFileOrganize, $post_title);
			return $result;
		} 
	} 
	public static function down_remote_img_by_wp($post_title, $url, $minWidth, $downImgTimeOut = 120, $relativeURL = 0, $downFileOrganize = 0) {
		global $wp_version;
		...
		if (in_array ($file_ext['ext'], $allowed_filetype)) {
			...
			$or_filename = basename($remote_image_url);
			$result = self :: handle_upload_img ($or_filename, $file_content, $mime, $file_ext['ext'], $relativeURL, $downFileOrganize, $post_title);
			return $result;
		} 
	} 
		...
		try {
			...
			if (isset($re -> error_code)) {
				...
				switch ($re -> error_code) {
					case '52001': $translated['err'] .= '[The translate text is too long]';
						break;
